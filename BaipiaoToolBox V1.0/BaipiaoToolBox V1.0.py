...
entry1 = tk.Entry(upper_frame, font=('微软雅黑', 13), show=None)
entry1.insert('end', '在此输入云盘链接')
entry1.place(relwidth=0.65, relheight=1)

button1 = tk.Button(upper_frame, font=('微软雅黑', 13), text='获取', command=lambda :getShareCode(entry1.get()))
button1.place(relx=0.7, relheight=1, relwidth=0.3)

entry2 = tk.Entry(lower_frame, font=('微软雅黑', 13))
entry2.insert('end', '在此输入vip视频链接')
entry2.place(relwidth=0.65, relheight=1)

button2 = tk.Button(lower_frame, font=('微软雅黑', 13), text='解析', command=lambda :AnalyzeVideo(entry2.get()))
button2.place(relx=0.7, relheight=1, relwidth=0.3)

label = tk.Label(root, bg='white', font=('微软雅黑', 13))
